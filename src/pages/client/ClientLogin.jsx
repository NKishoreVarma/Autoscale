...
export default function ClientLogin() {
  ...
  const { login, loginWithGoogle, user, userRole, loading: authLoading, authError, clearAuthError } = useAuth();
  const navigate = useNavigate();

  // Redirect based on role
  useEffect(() => {
    console.log("[ClientLogin] Auth state check — loading:", authLoading, "user:", user?.email || "null", "userRole:", userRole);
    if (!authLoading && user) {
      if (userRole === 'client') {
        console.log("[ClientLogin] User is client — redirecting to /client/dashboard");
        navigate('/client/dashboard', { replace: true });
      } else if (userRole && userRole !== 'client') {
        console.log("[ClientLogin] User is admin — redirecting to /admin");
  ...
  const handleEmailLogin = async (e) => {
    ...
    try {
      ...
      console.error("[ClientLogin] Email sign-in error:", err);
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      console.error("[ClientLogin] Google sign-in error:", err);
      setError(`Google Sign-in failed: ${err.message || 'Could not verify account.'}`);
    } finally {
      setLoading(false);
    }
  };
        ...
        {activeTab === 'google' && (
          <div className="space-y-6 py-4">
            <p className="text-sm text-gray-400 text-center font-light leading-relaxed">
              Log in securely using your registered Google email address.
            </p>
            <button
